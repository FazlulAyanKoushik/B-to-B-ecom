...
from addressio.models import Address, Division, Upazila, District
...
from orderio.models import Cart, CartProduct, Order, OrderProduct, OrderDelivery
...
def generate_avatar_image() -> InMemoryUploadedFile:
    ...
    static_folder = os.path.join(settings.REPO_DIR, "media/avatar")
    image_files = [
        f
...
def generate_product_image() -> InMemoryUploadedFile:
    # Get the path to the static folder
    static_folder = os.path.join(settings.REPO_DIR, "media/medicine")
    image_files = [
        f
        for f in os.listdir(static_folder)
        if f.endswith(".jpg") or f.endswith(".png") or f.endswith(".gif")
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            org1 = Organization.objects.create(name="lalmatia pharma", kind="Supplier")
            sazzad_islam = User.objects.create_superuser(
                phone="+8801755972192",
                password="123456",
                first_name="Sazzad",
                last_name="Islam",
            ...
            OrganizationUser.objects.create(
                organization=org1, user=sazzad_islam, role=OrganizationUserRole.CUSTOMER
            )
            super1 = User.objects.create_superuser(
                phone="+8801752495467",
                password="123456",
                first_name="Admin Saif",
                last_name="shahriar",
                email="",
            )
            ...
            merchants_id = []
            admins_id = []
            dis = District.objects.create(name="dhaka")

            def create_address(users: User):
                division = Division.objects.create(name=fake.city())
                district = District.objects.create(name=fake.city(), division=division)
                upazila = Upazila.objects.create(
                    name=fake.city(), district=district, division=division
                )

                Address.objects.create(
                    user=users,
                    label=fake.name(),
                    house_street=f"{fake.building_number()} {fake.city()}",
                    district=district,
                    division=division,
                    upazila=upazila,
                )

            def return_address_json(address: Address):
                return {
                    "uid": str(address.uid),
                    "label": address.label,
            ...
            def create_address_org(org: Organization):
                division = Division.objects.create(name=fake.city())
                district = District.objects.create(name=fake.city(), division=division)
                upazila = Upazila.objects.create(
                    name=fake.city(), district=district, division=division
            ...
            # creating real user
            user1_customer = User.objects.create_user(
                first_name="Sazzad",
                last_name="Customer",
                phone="+8801771845546",
                password="123456",
            ...
            create_address(users=super1)
            create_address(users=super1)
            create_address(users=super1)
            create_address(users=super1)

            user1_customer.image = generate_avatar_image()
            user1_customer.save()
            create_address(users=user1_customer)
            create_address(users=user1_customer)

            for i in range(80):
                name = fake.name()  # Generate a fake name
                phone_number = self.generate_bangladeshi_phone_number()
                password = "123456"
                ...
                user.save()
                create_address(users=user)
                users_id.append(user.id)
                create_address(users=user)

                OrganizationUser.objects.create(
                    organization=org1, user=user, role=OrganizationUserRole.CUSTOMER
                )
            ...
            OrganizationUser.objects.create(
                organization=org1, user=merchant1, role="OWNER", is_default=True
            )
            OrganizationUser.objects.create(
                organization=org1, user=super1, role="OWNER", is_default=True
            )
            create_address_org(org=org1)
            ...
            dosages = ["dosage_1", "dosage_2", "dosage_3", "dosage_4", "dosage_5"]
            manufacturers = [
                "manufacturers1",
                "manufacturers2",
                "manufacturers3",
            ...
            medicinePhysicalStates = [
                "medicinePhysicalStates",
                "medicinePhysicalStates1",
                "medicinePhysicalStates2",
                "medicinePhysicalStates3",
                "medicinePhysicalStates4",
            ]
            routeOfAdministrations = [
                "routeOfAdministrations",
                "routeOfAdministrations1",
                "routeOfAdministrations3",
                "routeOfAdministrations2",
                "routeOfAdministrations4",
            ]
            for category in categories:
                cat = Category.objects.create(name=category)
                categories_id.append(cat.id)
            for dos in dosages:
                dosage = DosageForm.objects.create(name=dos)
                dosage_form_id.append(dosage.id)
            for i in manufacturers:
                manu = Manufacturer.objects.create(name=i)
                Manufacturer_id.append(manu.id)
            for i in suppliers:
                supp = Supplier.objects.create(name=i)
                suppliers_id.append(supp.id)
            for i in medicinePhysicalStates:
                mps = MedicinePhysicalState.objects.create(name=i)
                medicinePhysicalStates_id.append(mps.id)
            for i in routeOfAdministrations:
                roa = RouteOfAdministration.objects.create(name=i)
                routeOfAdministrations_id.append(roa.id)
            for brand in brands:
                brands = Brand.objects.create(name=brand)
                brands_id.append(brands.id)
            for ingredient in ingredients:
                ing = Ingredient.objects.create(name=ingredient)
            ...
            for i in range(80):
                base_product = BaseProduct.objects.create(
                    ...
                    dosage_form_id=random.choice(dosage_form_id),
                ...
                for j in range(2):
                    base_product.active_ingredients.add(ing_add[j])
                for _ in range(3):
                    media = MediaImage.objects.create(image=generate_product_image())
                    MediaImageConnector.objects.create(
                        image=media, base_product=base_product
                    )
            base = list(
                set(
                    random.choices(
                        BaseProduct.objects.filter().values_list("id", flat=True), k=50
                    )
                )
            )
            ...
            for i in base:
                pr = Product.objects.create(
                    ...
                    base_product_id=i,
                    stock=100,
                    selling_price=random.randint(100, 900),
                    buying_price=random.randint(100, 900),
                    organization=merchant1.get_organization(),
                )
                product_id.append(pr.id)
            cartuser = list(set(random.choices(users_id, k=70)))
            cartuser.append(user1_customer.id)
            pay = PaymentMethod.objects.create(name="cash on delivery")

            for i in cartuser:
                cusUser = User.objects.get(id=i)
                cart, createdd = Cart.objects.get_or_create(
                    customer=cusUser, organization=org1
                )
                cartnum = random.randint(2, 9)
                prodnum = list(set(random.choices(product_id, k=cartnum)))

                for prodid in prodnum:
                ...
                if fake.pybool():
                    addre = cusUser.address_set.first()
                    addressjson = return_address_json(address=addre)
